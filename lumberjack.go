...
func Init(mysqlConf *MySQLConfig) {
	...
	// 验证连接
	if err != nil {
		fmt.Printf("get db connetion failed,url=[%s]err=[%v]\n", url, err)
		panic(err)
	}
	...
	if err != nil {
		panic(fmt.Errorf("err: %v, sql: %s", err, sqlStr))
	}
}
	...
	if err != nil {
		...
		if err == sql.ErrNoRows {
			...
			if err := insertFileMetaInfo(name, dbDate, dbIndex, db, tb); err != nil {
				//				log.Printf("insert file: %s meta info to mysql err: %v", filename, err)
				return "", "", -1, fmt.Errorf("insert file: %s meta info to mysql err: %v", filename, err)
			}
			return filepath.Join(dir, fmt.Sprintf("%s-%s-%04d%s", prefix, date, index, ext)), date, index, nil
		} else {
			// 说明数据表查询出错
			//			log.Printf("get file: %s meta info from mysql err: %v", filename, err)
			return "", "", -1, fmt.Errorf("get file: %s meta info from mysql err: %v", filename, err)
		}
	}
	index = meta.index
	date = meta.date
...
func getFileMetaInfo(dbStr, tbStr string) (*fileMetaInfo, error) {
	...
	fmi.db = dbStr
	fmi.tb = tbStr
	err = db.QueryRow("SELECT date, idx FROM t_binlog2file_status WHERE db = ? AND tb = ?;", dbStr, tbStr).Scan(&fmi.date, &fmi.index)
	return &fmi, err
}
...
func getFileMetaInfos() (fmis []*fileMetaInfo, err error) {
	...
	rows, err = db.Query("SELECT date, idx, db, tb FROM t_binlog2file_status")
	if err != nil {
		return fmis, err
	}
	defer rows.Close()
